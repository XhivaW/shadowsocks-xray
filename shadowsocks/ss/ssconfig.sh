...
creat_v2ray_json() {
	...
	if [ "$ss_basic_v2ray_use_json" == "0" ]; then
		...
		xtls) # TODO: support
			...
			local vless_flow="\"flow\": \"$ss_basic_xray_network_flow\""
			;;
		*)
			local tls="null"
			local xtls="null"
...
start_v2ray_xray() {
	# DEBUG
	echo_date "!!!!!!!!!!!!!!!!!"
	echo_date "$ss_basic_v2ray_xray"
	echo_date "!!!!!!!!!!!!!!!!!"
	if [ "$ss_basic_v2ray_xray" == "xray" ]; then
		start_xray
	elif [ "$ss_basic_v2ray_xray" == "v2ray" ]; then
		start_v2ray
	else
		echo_date "ss_basic_v2ray_xray定义错误！当前值为：$ss_basic_v2ray_xray，期望值为 xray 或 v2ray"
		disable_ss
		close_in_five
	fi
}
