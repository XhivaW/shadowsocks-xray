...
get_model(){
	...
	local PRODUCTID=$(nvram get productid) # RT-AX88U
	if [ -n "${ODMPID}" ];then
		MODEL="${ODMPID}"
	else
		MODEL="${PRODUCTID}"
	fi
	# return MODEL="RT-AX88U"
}
...
install_now(){
	...
	rm -rf /koolshare/bin/xray # for xray support
	...
	sync # write what in buff into disk
	...
	# 设置xray 版本号
	dbus set ss_basic_xray_version="v1.5.3"
